...
$(document).ready(function(){
    ...
    $('.btn-reject-app').click(function(){
        if (confirm('Are you sure? This can\'t be undone.')) {
            ...
            $.post('/TutorsHub/assignment/reject?id=' + id,null,function(data){
                if (data.status == 1) {
...
function addFavorite(tutorId){
    $.post('/TutorsHub/requestor/ajaxAddFavorite', {tutorId: tutorId}, function(data){
        if (data.status) {
...
function removeFavorite(tutorId){
    $.post('/TutorsHub/requestor/ajaxRemoveFavorite', {tutorId: tutorId}, function(data){
        if (data.status) {
...
function acceptApplication(appId) {
    ...
    $.post('/TutorsHub/assignment/accept?id=' + appId,null,function(data){
        if (data.status == 1) {
