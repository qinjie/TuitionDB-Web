...
$(document).ready(function(){
    ...
    if ($('#category-select').length > 0) {
        ...
        var subjects;
        $.get('/ajax/subjectTree',function(data){
            subjects = data;
    ...
    if ($('#mrt-line-select').length > 0) {
        var stations;
        $.get('/ajax/stationTree',function(data){
            stations = data;
    ...
    if ($('#weekday-select').length > 0) {
        ...
        var slots;
        $.get('/ajax/scheduleTree',function(data){
            slots = data;
    ...
    if ($('#weekday-select-new').length > 0) {
        var allSlots;
        $.get('/ajax/scheduleTreeNew',function(data){
            allSlots = data;
        });
        $('#weekday-select-new, #timeslot-select-new, #btn-add-slot-new').click(function(){
            if ($('#weekday-select-new').val() == null || $('#timeslot-select-new').val() == null) {
                $('#btn-add-slot-new').attr({
                    "data-toggle": "tooltip",
                    "data-original-title": "Please select weekday and timeslot",
                    "data-placement": "right",
                });
                $('#btn-add-slot-new').tooltip('show');
            } else {
                $('#btn-add-slot-new').tooltip('destroy');
            }
        });
    ...
    if ($('#student-info-form').length > 0) {
        ...
        var schoolList;
        $.get('/ajax/schoolList',function(data){
            schoolList = data;
